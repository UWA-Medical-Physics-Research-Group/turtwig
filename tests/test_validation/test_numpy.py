...
import numpy as np
import pytest
from pydantic import ValidationError, validate_call, AfterValidator
...
class TestIsNdim:
    ...
    def test_validate_using_pydantic(self):
        @validate_call()
        def func(arr: Annotated[np.ndarray, NumpyArray, AfterValidator(is_ndim(ndim=3))]):
            return arr
    ...
    def test_multiple_ndims(self):
        @validate_call()
        def func(arr: Annotated[np.ndarray, NumpyArray, AfterValidator(is_ndim(ndim=[2, 5]))]):
            return arr
